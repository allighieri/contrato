...
$idAparelhos = isset($_GET['id']) ? $_GET['id'] : "";
	
$sql= "SELECT * FROM aparelhos WHERE idAparelhos = :idAparelhos";
...
<body>	
	...
	<section>
		...
		<form id="form_insere_aparelhos" name="form" autocomplete="off">
			<h2>Cadastrar Itens:</h2>
			
			<input id="idAparelhos" type="hidden" name="idAparelhos" value="<?php echo $idAparelhos; ?>"/>
			<input id="acao" type="hidden" name="acao" value="editar" placeholder=" "/>			
			
			<div class="col2x-left">
				<div class="label-float">
					<input id="nome" type="text" name="nome" value="<?php echo isset($linhasAparelhos['nome']) ? $linhasAparelhos['nome'] : "" ;?>" placeholder=" "/>
					 <label>Nome</label>
				</div>
			</div>
			...
			<div class="col2x-right">
				<div class="label-float">
					<input id="modelo" type="text" name="modelo" value="<?php echo isset($linhasAparelhos['modelo']) ? $linhasAparelhos['modelo'] : "" ;?>" placeholder=" "/>
					 <label>Modelo</label>
				</div>
			</div>			
			...
			<div class="col2x-left">
				<div class="label-float">
					<input id="serial" type="text" name="serial" value="<?php echo isset($linhasAparelhos['serial']) ? $linhasAparelhos['serial'] : "" ;?>" placeholder=" "/>
					 <label>Serial</label>
				</div>
			</div>
			...
			<div class="col2x-right">
				<div class="label-float">
					<input id="musicas" type="text" name="musicas" value="<?php echo isset($linhasAparelhos['musicas']) ? $linhasAparelhos['musicas'] : "" ;?>" placeholder=" "/>
					 <label>Músicas</label>
				</div>
			</div>	
			...
			<div class="checkbox">
				<label> Item padrão: </label>
				  <input type="radio" id="padraosim" name="padrao" value="1" <?php echo $linhasAparelhos['padrao'] == 1 ? "checked" : ""; ?> /> <label class="for" for="padraosim"> Sim</label></span>
				  <input type="radio" id="padraonao" name="padrao" value="0" <?php echo $linhasAparelhos['padrao'] == 0 ? "checked" : "";?>  /> <span><label class="for" for="padraonao"> Não</label></span>
			</div>
...
<script>
	...
	//INSERE EVENTO
	$(document).on('submit','#form_insere_aparelhos',function(){
		

		var dados = new FormData(this); //pega os dados do form e cria um array
		
		var idAparelhos = $("#idAparelhos").val();
		...
		var musicas = $("#musicas").val();
		var musicas = musicas.trim();
		
		
		
		var padrao = $('input[name="padrao"]:checked').val();
		
		

		if(nome == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Descreva o nome do videokê</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}	

		if(modelo == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Descreva o modelo do videokê</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}	
		
		if(serial == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Informe o serial do videokê</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}

		if(musicas == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Informe a quantidade de músicas do videokê</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}		
						...
						if(res.status == 1){
							...
							$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
								...
								$('.success').on('click', function(){
									window.location.href = "listar_aparelhos.php";
								});								
