...
$sql= "SELECT 
			...
			INNER JOIN aparelhos_contrato as ac ON ac.idEventos = e.idEventos
			INNER JOIN itens_contrato as ic ON ic.idEventos = e.idEventos GROUP BY ic.idEventos
			";
...
<body>	
	<header>
		<div id="topo">
			...
			<h1>Lista de Contratos (Itens)</h1>
		</div>
	...
	<a href="form_itens_contrato.php" class="btn_cadastrar_novo" title="Cadastrar um novo template">NOVO ITEM</a>
	<div class="clear"></div>
	...
	<table id="example" class="display nowrap" style="width:100%">
        ...
        <tbody>
			...
			<tr>
                ...
                <td class="editavel" title="Status" data-id="<?php echo $eventos['idEventos']; ?>"> <?php echo $eventos['nomeStatus']; ?> </td>
                
				<td style="text-align:center;">
					<a href="form_editar_itens_contrato.php?id=<?php echo $eventos['idEventos'] ;?>&idAparelhos=<?php echo $eventos['idAparelhos'] ;?>" title="Editar">
				...
				<a href="gerar_contrato.php?id=<?php echo $eventos['idEventos'] ;?>" title="Gerar Contrato" target="_blank">
						<i class="fa fa-file-text-o" aria-hidden="true"></i></a> |
			

					<a href="<?php echo $eventos['idEventos'] ;?>" title="Enviar Contrato por Email" target="_blank" data-id="<?php echo $eventos['idStatus'] ;?>" class="enviarEmail">
						<i class="fa fa-share-square-o" aria-hidden="true"></i></a> |
						
					
					<a href="itens_contrato.php?id=<?php echo $eventos['idEventos'] ;?>&acao=deletar" title="Deletar Itens do Contrato">
...
$('#example tbody tr td.editavel').on('dblclick',
    function() {
      ...
      novoElemento.on('change', function(){
        
		conteudoOriginal = $(this.name + " option:selected").text();
		
		console.log(conteudoOriginal);
		
 				
      });
      $(this).html(novoElemento.on('blur keydown', function(e) {
        var keyCode = e.which;
        ...
        if (keyCode == 13 && conteudoNovo != '' && conteudoNovo != conteudoOriginal) {
          var objeto = $(this);
			...
			$.ajax({
				...
				url: "teste.php",
				data: {
				  idEventos: $(this).parent().attr('data-id'),
				  idStatus: conteudoNovo
				}, //added this comma here
				success: function(result) {
				  objeto.parent().html(conteudoOriginal);
				  $('body').append(result);
				}
			  })
...
$('.enviarEmail').on('click', function(e){
	...
	var idEventos = $(this).attr('href');
		...
		$.ajax({
			url: 'email.php',
			type: "POST",
			data: {'id':id, 'idEventos':idEventos},
			beforeSend: function() {
