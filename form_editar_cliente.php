...
<body>	
	...
	<section>
		...
		<form id="form_insere_cliente" name="form" autocomplete="off">
			<h2>Dados do Cliente</h2>
			
			
			<input id="idCpfCnpj" type="hidden" name="idCpfCnpj" value="<?php echo $linhasCliente['idCpfCnpj']; ?>" placeholder=" "/>
			<input id="acao" type="hidden" name="acao" value="editar" placeholder=" "/>
			<input id="dataCPF" type="hidden" name="dataCPF" value="<?php echo $linhasCliente['data']; ?>" placeholder=" "/>
			
			<div class="label-float">
				<input id="cpf_cnpj" type="text" name="cpf_cnpj" value="<?php echo $linhasCliente['cpf_cnpj']; ?>" placeholder=" " />
				 <label>CPF/CNPJ</label>
				 <span class="check"></span>
			</div>

			<div class="label-float">
				<input id="nome" type="text" name="nome" value="<?php echo $linhasCliente['nome']; ?>" placeholder=" "/>
				 <label>Nome/Razão Social</label>
			</div>			
			
			<div class="col2x-left">			
				<div class="label-float">
					<input id="telefone" type="text" name="telefone" value="<?php echo $linhasCliente['telefone']; ?>" placeholder=" "/>
			...
			<div class="col2x-right">						
				<div class="label-float">
					<input id="email" type="email" name="email" value="<?php echo $linhasCliente['email']; ?>" placeholder=" "/>
					 <label>E-mail</label>
				</div>
			</div>

			<div class="label-float">
				<input id="cep" type="text" name="cep" value="<?php echo $linhasCliente['cep']; ?>" placeholder=" "/>
				 <label>CEP</label>
			</div>


			<div class="label-float">
				<input id="endereco" type="text" name="endereco" value="<?php echo $linhasCliente['endereco']; ?>" placeholder=" "/>
				 <label>Endereço</label>
			</div>
			
			<div class="label-float">
				<input id="bairro" type="text" name="bairro" value="<?php echo $linhasCliente['bairro']; ?>" placeholder=" "/>
				 <label>Bairro</label>
			</div>			

			<div class="col2x-left">			
				<div class="label-float 2x">
					<input id="cidade" type="text" name="cidade" value="<?php echo $linhasCliente['cidade']; ?>" placeholder=" "/>
					 <label>Cidade</label>
				</div>	
			</div>	
			...
			<div class="col2x-right">			
				<div class="label-float">
					<input id="uf" type="text" name="uf" value="<?php echo $linhasCliente['uf']; ?>" placeholder=" "/>
					 <label>UF</label>
				</div>			
			</div>	
...
$(document).ready(function(){
		...
		$(".label-float input#email").css("text-transform", "none");
	...
	$(function () {
		...
		$("#cpf_cnpj").on('keyup keypress blur change',function () {
			...
			var verificaCPF = function(){
				
				var minlength = 14;
				
				var value = $('#cpf_cnpj').val();
				var valueCPF = "<?php echo $_GET['cpf'] ?>";
				...
				if (value.length >= minlength ) {
					if (searchRequest != null) 
						...
						success: function(data){
							...
							if(res.status == 1){ // se houver um cpf/cnpj já cadastrado mostra um erro e desabilita o botão de cadastrar
								...
								if (value.length >= 15 && value.length <= 17 ){
									...
									$(".check").fadeTo(100, 1).html('<span class="errado" title="CPF/CNPJ ' + res.cpf_cnpj + ' já está associado a um cliente">X</span>');
									$('#btn_enviar').addClass('disabled');
									$('#btn_enviar').prop("disabled", true);
								}
							...
							if(res.status == 0){ // se não houver um cpf/cnpj já cadastrado mostra um "ok" e habilita o botão de cadastrar
								...
								if (value.length >= 15 && value.length <= 17 ){
									...
									$(".check").fadeTo(100, 1).html('<span class="correto"  title="CPF/CNPJ  ' + res.cpf_cnpj + ' livre para cadastro">&#10004;</span>');
	...
	$(document).on('submit','#form_insere_cliente',function(){
		...
		var cpf_cnpj = cpf_cnpj.trim();
		var nome = nome.trim();
		var telefone = telefone.trim();
		...
		//função que verifica se um nome é completo e válido
		...
		//chama a função que verifica se o nome é completo
		if(resultados == 'false'){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Digite seu nome COMPLETO corretamente.</p><span class="fechar">X</span></span>');
		...
		if(telefone == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe um telefone de contato.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		
		if (!validaEmail(email)) {
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Digite um e-mail válido.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		...
		if(bairro == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe seu bairro.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		...
		if(cpf_cnpj.length == 14 || cpf_cnpj.length == 18){
			...
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Informe um CPF/CNPJ válido. ' + cpf_cnpj.length + ' </p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;
		}
