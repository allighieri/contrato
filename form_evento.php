...
<?php
	...
	if(isset($_POST['texto']) && !empty($_POST['texto'])){
		...
		$texto = $_POST['texto'];
		$texto = filter_var(mb_strtoupper($texto,'utf-8'));		
		...
		$instalacao = isset($final_data['DATA']) ? $final_data['DATA'] : "";		
		$instalacao = DateTime::createFromFormat("d/m/Y", $instalacao);
		
		$retirada = isset($final_data['DATA']) ? $final_data['DATA'] : "";
		$retirada = DateTime::createFromFormat("d/m/Y", $retirada);
		$retirada->add(new DateInterval('P1D')); // P1D means a period of 1 day
...
<body>	
	...
	<section>
		...
		<form id="form_insere_evento" name="form" autocomplete="off">
			<h2>Cadastrar Evento para:</h2>
			
			<input id="idCliente" type="hidden" name="idCliente" value=""/>

			
			<div class="label-float">
				<input type="text" id="gsearchsimple" name="gsearchsimple" placeholder="Search..." />
			...
			<div class="label-float">
				<input id="cpf_cnpj" type="text" name="cpf_cnpj" value="<?php echo isset($final_data['CPF']) ? $final_data['CPF'] : ""; ?>" placeholder=" " disabled />
			...
			<div class="label-float">
				<input id="cep" type="text" name="cep" value="<?php echo isset($final_data['CEP']) ? $final_data['CEP'] : ""; ?>" placeholder=" "/>
				 <label>CEP</label>
			</div>

			<div class="label-float">
				<input id="endereco" type="text" name="endereco" value="<?php echo isset($final_data['ENDEREÇO DA FESTA']) ? $final_data['ENDEREÇO DA FESTA'] : ""; ?>" placeholder=" "/>
				 <label>Endereço</label>
			</div>
		
	
			<div class="label-float">
				<input id="bairro" type="text" name="bairro" value="<?php echo isset($final_data['BAIRRO']) ? $final_data['BAIRRO'] : ""; ?>" placeholder=" "/>
				 <label>Bairro</label>
			</div>

			<div class="col2x-left">				
				<div class="label-float">
					<input id="cidade" type="text" name="cidade" value="<?php echo isset($final_data['CIDADE']) ? $final_data['CIDADE'] : ""; ?>" placeholder=" "/>
					 <label>Cidade</label>
				</div>
			</div>	
			
			<div class="col2x-right">
				<div class="label-float">
					<input id="uf" type="text" name="uf" value="<?php echo isset($final_data['UF']) ? $final_data['UF'] : ""; ?>" placeholder=" "/>
					 <label>UF</label>
				</div>
			</div>	
			...
			<div class="col2x-left">				
				<div class="label-float">
					<input id="instalacao" type="date" name="instalacao" value="<?php echo isset($final_data['DATA']) ? $instalacao->format("Y-m-d") : date('Y-m-d'); ?>" placeholder=" "/>
					 <label class="label_instalacao">Instalação</label>
				</div>			
			</div>
			
			<div class="col2x-right">
				<div class="label-float">
					<input id="horainstalacao" type="time" step="0" name="horainstalacao" value="<?php echo date('H:i'); ?>" placeholder=" "/>
			...
			<div class="col2x-left">				
				<div class="label-float">
					<input id="retirada" type="date" name="retirada" value="<?php echo isset($final_data['DATA']) ? $retirada->format('Y-m-d') : date("Y-m-d", strtotime(date('Y-m-d').'+ 1 days')); ?>" placeholder=" "/>
			...
			<div class="col2x-right">
				<div class="label-float">
					<input id="horaretirada" type="time" step="0" name="horaretirada" value="<?php echo date('H:i'); ?>" placeholder=" "/>
 ...
 $(document).on('click', '.gsearch', function(){
  ...
  $.ajax({
   ...
   success:function(data)
   {
	...
	//verifica se foi diditado um cpf ou cnpj para fazer a formatação com jqueyr mask
	...
	$(document).on('submit','#form_insere_evento',function(){
		...
		var gsearchsimple = $('#gsearchsimple').val();
		...
		var horainstalacao = $('#horainstalacao').val();
		var retirada = $('#retirada').val();
		var horaretirada = $('#horaretirada').val();
		...
		var uf = uf.trim();
		var instalacao = instalacao.trim();
		var horainstalacao = horainstalacao.trim();
		var horainstalacao = horainstalacao.trim();
		var retirada = retirada.trim();
		var horaretirada = horaretirada.trim();
		var idStatus = idStatus.trim();
		
		

		if(gsearchsimple == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Para quem será esse evento?.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}		

		if(cep == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe o CEP.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}		
		
		if(endereco == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe seu endereço.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		
		if(bairro == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe o bairro.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}			
		
		if(cidade == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe a cidade.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}		
		...
		if(uf == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, o Estado com duas letras.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		
		if(instalacao == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe a data de instalação.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		
		if(horainstalacao == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, a hora da instalação.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}

		if(retirada == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, informe a data da retirada.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}

		if(horaretirada == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, a hora da retirada.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}

		if(idStatus == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Por favor, defina um status.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}		
