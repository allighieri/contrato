...
<body>	
	...
	<section>
	    <form id="form_pesquisa_cliente" name="form" autocomplete="off">
			<h2>Pesquisa Cliente</h2>
			
			<div class="label-float">
			  <input id="cpf_cnpj" type="text" name="cpf_cnpj" value="" placeholder=" "/>
			  <label>CPF/CNPJ</label>
			</div>
			
			<input type="submit" name="btn_enviar" value="Buscar" id="btn_enviar" class="btn"/>
		...
		<form id="form_insere_cliente" name="form" autocomplete="off">
			...
			<div class="label-float">
				<input id="cpf_cnpj_cliente" type="text" name="cpf_cnpj_cliente" value="" placeholder=" " />
				 <label>CPF/CNPJ</label>
			</div>
			...
			<div class="label-float">
				<input id="telefone" type="text" name="telefone" value="" placeholder=" "/>
				 <label>Telefone</label>
			</div>
			
			<div class="label-float">
				<input id="endereco" type="text" name="endereco" value="" placeholder=" "/>
			...
			<input type="submit" name="btn_enviar" value="Cadastrar" id="btn_enviar" class="btn"/>
...
<script>
	...
	//Pesquisa Cliente
	$(document).on('submit','#form_pesquisa_cliente',function(){
		...
		$.ajax({
			...
			success: function( data ) {
				...
				if(res.status == 1){
					$(".status").fadeIn( "slow").html('<span class="success"><p>'+ res.erro +'</p><span class="fechar">X</span></span>');
					$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
					
					$('#nome').val(res.nome);
					$('#telefone').val(res.telefone);
					$('#cpf_cnpj_cliente').val(res.cpf_cnpj);
				...
				if(res.status != 1){
					$(".status").fadeIn( "slow").html('<span class="danger"><p>'+ res.erro +'</p><span class="fechar">X</span></span>');
					$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
				}
	...
	$(document).on('submit','#form_insere_cliente',function(){
		...
		/*function valida(nome_completo) {
		  return !!nome_completo.match(/^[A-ZÀ-Ÿ][A-zÀ-ÿ']+\s([A-zÀ-ÿ']\s?)*[A-ZÀ-Ÿ][A-zÀ-ÿ']+$/);
		}	
		

		const testes = [nome_completo];
		const resultados = testes.map(valida);	
		
		*/
		
		if (!validaEmail(email)) {
			$(".status").fadeIn( "slow").html('<span class="danger"><p>Digite um e-mail válido.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		...
		/*
		if(resultados == 'false'){
			$(".status").fadeIn( "slow").html('<span class="danger"><p>Digite seu nome completo corretamente.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;
		}	
		*/
