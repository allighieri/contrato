...
<?php
	...
	$idItens = isset($_GET['id']) ? $_GET['id'] : "";
	
		//lista os itens
		$sql= "SELECT * FROM itens WHERE idItens = :idItens";
		$stmt = $pdo->prepare($sql);
		$stmt->bindParam(':idItens', $idItens, PDO::PARAM_INT);		
		$stmt->execute();	
		$total = $stmt->rowCount();	
		$linhasItens = $stmt->fetch();		
...
<body>	
	...
	<section>
		...
		<form id="form_editar_itens" name="form" autocomplete="off">
			<h2>Editar Itens:</h2>
			
			<input id="idItens" type="hidden" name="idItens" value="<?php echo $idItens; ?>"/>
			<input id="acao" type="hidden" name="acao" value="editar" placeholder=" "/>
			
			<div class="col2x-left">
				<div class="label-float">
					<input id="descricao" type="text" name="descricao" value="<?php echo isset($linhasItens['descricao']) ? $linhasItens['descricao'] : "" ;?>" placeholder=" "/>
					 <label>Item</label>
				</div>
			</div>
			...
			<div class="col2x-right">
				<div class="label-float">
					<input id="qtde" type="number" name="qtde" value="<?php echo isset($linhasItens['qtde']) ? $linhasItens['qtde'] : "" ;?>" placeholder=" "/>
			...
			<div class="checkbox">
				<label> Item padrão: </label>
				  <input type="radio" id="padraosim" name="padrao" value="1" <?php echo $linhasItens['padrao'] == 1 ? "checked" : ""; ?> /> <label class="for" for="padraosim"> Sim</label></span>
				  <input type="radio" id="padraonao" name="padrao" value="0" <?php echo $linhasItens['padrao'] == 0 ? "checked" : "";?>  /> <span><label class="for" for="padraonao"> Não</label></span>
			</div>
...
<script>
	...
	//INSERE EVENTO
	$(document).on('submit','#form_editar_itens',function(){
		...
		if(descricao == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Descreva o item para inserir.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}	

		if(qtde == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Quantidade inválida.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
		
		if(qtde < 1){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Quantidade inválida, insira um valor igual ou maior que 1.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		}
				...
				console.log(res.erro);
						...
						if(res.status == 1){
							...
							$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
								...
								$('.check').fadeOut("fast");
								$('#form_editar_itens input').val("");
								$('#btn_enviar').val("Cadastrar");
								
								$('.success').on('click', function(){
									window.location.href = "listar_itens.php";
								});
