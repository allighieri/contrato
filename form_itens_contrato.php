...
<body>	
	...
	<section>
		...
		<form id="form_insere_evento" name="form" autocomplete="off">
			...
			<div class="label-float">
				<input id="dataInstalacao" type="text" name="dataInstalacao" value="" placeholder=" " disabled />
			...
			<div class="checkbox">
				...
				<?php foreach($linhasAparelhos as $linhasAparelhos){ ?>
					<input type="radio" id="aparelhos<?php echo $linhasAparelhos['idAparelhos']?>" name="aparelhos" value="<?php echo $linhasAparelhos['idAparelhos'] ;?>" <?php echo $linhasAparelhos['padrao'] == 1 ? "checked" : ""; ?> />  
					<span class="descAparelho"><label class="for" for="aparelhos<?php echo $linhasAparelhos['idAparelhos']?>"><?php echo $linhasAparelhos['nome']?> <?php echo $linhasAparelhos['musicas']?> músicas</label></span>
				<?php } ?>  
			</div>
  ...
  $.ajax({
   ...
   success:function(data)
   {
	...
	$(document).on('submit','#form_insere_evento',function(){
		...
		//cria o array para as quantidades e descriç~eos
		var idItens = [];
		var qtde = [];
		...
		if(valorDiaria == ''){
			$(".status").fadeTo(100, 0.85).html('<span class="danger"><p>Informe o valor da locação.</p><span class="fechar">X</span></span>');
			$('.fechar').on('click', function(){ $('.status').fadeOut("slow"); });
			return false;				
		};			
